...
type UserRow = {
  ...
  puesto: { id: number; nombre: string; rol: { nombre: string } | null } | null;
};
...
export default function RolesPermisosPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: basesData }, { data: puestosData }, viewerRes] =
        await Promise.all([
      ...
      const puestosRaw = (puestosData as unknown[] | null) ?? [];
      const puestosNormalized: PuestoRow[] = puestosRaw.map((p) => {
        const row = p as { id: unknown; nombre: unknown; rol?: unknown };
        const rolObj = normalizeJoin<{ nombre: string }>(row.rol);
        return {
          id: Number(row.id),
  ...
  useEffect(() => {
    ...
    async function loadUsers() {
      ...
      let q = supabase
        .from("profiles")
        .select(
          "user_id,nombre,apellido,telefono,base_id,puesto_id,base:bases(id,nombre),puesto:puestos(id,nombre,rol:roles(nombre))",
        )
      ...
      if (error) {
        ...
        setErrorMessage(
          "No se pudo cargar usuarios. Revisa RLS de profiles para rol Administrador.",
        );
        return;
      }

      const raw = (data as unknown[] | null) ?? [];
      const normalized: UserRow[] = raw.map((r) => {
        const row = r as {
          user_id: unknown;
          nombre?: unknown;
          apellido?: unknown;
        ...
        const baseObj = normalizeJoin<{ id: number; nombre: string }>(row.base);
        let puestoObj = normalizeJoin<{
          id: number;
          nombre: string;
          rol: unknown;
        }>(row.puesto);
        ...
        if (puestoObj) {
          const rolObj = normalizeJoin<{ nombre: string }>(puestoObj.rol);
          puestoObj = { ...puestoObj, rol: rolObj };
        }
        ...
        return {
          ...
          base_id:
            typeof row.base_id === "number"
              ? row.base_id
              : row.base_id == null
                ? null
                : Number(row.base_id),
          puesto_id:
            typeof row.puesto_id === "number"
              ? row.puesto_id
              : row.puesto_id == null
                ? null
                : Number(row.puesto_id),
          base: baseObj,
          puesto: (puestoObj as UserRow["puesto"]) ?? null,
        };
      });
  ...
  async function updateUserPuesto(userId: string, puestoId: number | null) {
    setErrorMessage("");
    setSaveMessage("");
    ...
    if (error) {
      setSavingUserId("");
      setErrorMessage(
        "No se pudo actualizar el rol/puesto. Revisa RLS de update en profiles para Administrador.",
      );
      return;
    }

    const newPuesto =
      puestoId == null ? null : puestos.find((p) => p.id === puestoId) ?? null;
    ...
    setUsers((prev) =>
      prev.map((u) => {
        ...
        return {
          ...
          puesto_id: puestoId,
          puesto: newPuesto
            ? { id: newPuesto.id, nombre: newPuesto.nombre, rol: newPuesto.rol }
            : null,
        };
      }),
    );
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-600">
                  Solo administradores pueden cambiar el rol (puesto) de los usuarios.
                </p>
              </div>
              ...
              {users.length > 0 ? (
                <div className="mt-6 overflow-hidden rounded-2xl border border-black/[.08]">
                  <div className="grid grid-cols-6 gap-0 bg-zinc-50 px-4 py-3 text-xs font-medium uppercase tracking-wide text-zinc-500">
                    ...
                    <div className="col-span-2">Rol / Puesto</div>
                  ...
                  <div className="divide-y divide-black/[.08] bg-white">
                    {users.map((u) => {
                      ...
                      return (
                        <div
                          key={u.user_id}
                          className="grid grid-cols-6 items-center gap-0 px-4 py-3 text-sm text-zinc-700"
                        >
                          ...
                          <div className="col-span-2">
                            ...
                            <select
                              value={u.puesto_id == null ? "" : String(u.puesto_id)}
                              onChange={(e) => {
                                const v = e.target.value;
                                void updateUserPuesto(
                                  u.user_id,
                                  v ? Number(v) : null,
                                );
                              }}
                              disabled={saving}
                              className="h-10 w-full rounded-xl border border-black/[.12] bg-white px-3 text-sm outline-none focus-visible:border-black/40 focus-visible:ring-2 focus-visible:ring-black/10 disabled:opacity-60"
                            >
                              <option value="">Sin puesto</option>
                              {puestos.map((p) => (
                                <option key={p.id} value={String(p.id)}>
                                  {p.rol?.nombre ? `${p.rol.nombre} · ` : ""}
                                  {p.nombre}
                                </option>
                              ))}
                            </select>
